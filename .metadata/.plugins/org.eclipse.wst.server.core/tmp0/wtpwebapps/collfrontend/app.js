 ...
 * $rootScope.$on('$locationChangeStart',function(event,next,current){ var
 * unRestrictedPage=$.inArray($location.path(),['/login','/register','/list_friend','/list_event','/list_job','/list_forum'])==0;
 * var loggedIn=$rootScope.currentUser.user_id; console.log("loggedIn:" +
 * loggedIn) if(unRestrictedPage && !loggedIn){ console.log("Navigating to login
 * page") $location.path('/login'); } });
 * 
 * //keep user logged in after page refresh
 * $rootScope.currentUser=$cookieStore.get('currentUser')||{};
 * if($rootScope.currentUser){ $http.defaults.headers.common['Authorization']
